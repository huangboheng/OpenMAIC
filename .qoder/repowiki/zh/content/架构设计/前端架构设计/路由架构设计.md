# 路由架构设计

<cite>
**本文引用的文件**   
- [app/layout.tsx](file://app/layout.tsx)
- [app/page.tsx](file://app/page.tsx)
- [app/classroom/[id]/page.tsx](file://app/classroom/[id]/page.tsx)
- [components/preview-expired-overlay.tsx](file://components/preview-expired-overlay.tsx)
- [lib/hooks/use-preview-timer.ts](file://lib/hooks/use-preview-timer.ts)
- [app/generation-preview/layout.tsx](file://app/generation-preview/layout.tsx)
- [app/generation-preview/page.tsx](file://app/generation-preview/page.tsx)
- [middleware.ts](file://middleware.ts)
- [next.config.ts](file://next.config.ts)
- [components/access-code-guard.tsx](file://components/access-code-guard.tsx)
</cite>

## 更新摘要
**已进行的更改**
- 新增课堂页面预览限制功能，包括10分钟免费试看计时器
- 实现课程slug提取和条件覆盖层渲染机制
- 添加PreviewExpiredOverlay组件用于超时阻断
- 集成usePreviewTimer Hook进行本地存储计时管理

## 目录
- 产品概述
- 核心业务流程
- 功能模块清单
- 数据与状态
- 关键约束与边界

## 产品概述
OpenMAIC 是基于 Next.js App Router 的 AI 交互式课件生成与课堂管理平台。路由体系围绕"首页入口 → 预览生成 → 教室播放"的主路径组织，配合 API 路由、动态路由参数、嵌套布局与中间件实现安全鉴权、限流与跨端协作。页面采用客户端组件与服务器渲染协同，结合代码分割与预取策略，保障首屏性能与交互流畅度。**新增预览限制功能**，在课堂页面提供10分钟免费试看体验，超时后显示阻断覆盖层引导用户返回课程页面。

## 核心业务流程
- 首页入口：用户输入需求、选择资料与模式后，进入预览生成页。
- 预览生成：流式生成大纲与媒体任务，支持人工审阅与自动继续。
- 教室播放：根据课程 ID 加载场景与智能体，恢复未完成的任务并渲染课堂。
- **预览限制**：课堂内10分钟免费试看计时，超时后显示阻断覆盖层。

```mermaid
sequenceDiagram
participant U as "用户"
participant H as "首页(app/page.tsx)"
participant GP as "预览生成(app/generation-preview/page.tsx)"
participant CL as "教室详情(app/classroom/[id]/page.tsx)"
participant MW as "中间件(middleware.ts)"
participant API as "API路由(/api/*)"
participant PT as "预览计时器(use-preview-timer.ts)"
U->>H : 填写需求/上传资料/选择模式
H->>GP : 写入会话到 sessionStorage 并跳转
GP->>API : 调用生成接口(受中间件限流与鉴权)
API-->>GP : 返回大纲/媒体任务(流式)
GP-->>U : 展示进度/可审阅/自动继续
GP->>CL : 完成或确认后跳转到教室
CL->>PT : 初始化10分钟免费试看计时
PT-->>CL : 返回计时状态(expired, remainingSeconds)
CL->>API : 加载教室数据/恢复任务
API-->>CL : 返回教室结构与媒体任务
CL-->>U : 渲染课堂并继续生成未完成任务
Note over PT,CL : 10分钟后显示PreviewExpiredOverlay覆盖层
Note over MW,API : 所有 /api/* 请求经中间件鉴权与限流
```

**图表来源** 
- [app/page.tsx:317-409](file://app/page.tsx#L317-L409)
- [app/generation-preview/page.tsx:187-200](file://app/generation-preview/page.tsx#L187-L200)
- [app/classroom/[id]/page.tsx:40-47](file://app/classroom/[id]/page.tsx#L40-L47)
- [lib/hooks/use-preview-timer.ts:23-76](file://lib/hooks/use-preview-timer.ts#L23-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)
- [middleware.ts:78-136](file://middleware.ts#L78-L136)

**章节来源**
- [app/page.tsx:317-409](file://app/page.tsx#L317-L409)
- [app/generation-preview/page.tsx:187-200](file://app/generation-preview/page.tsx#L187-L200)
- [app/classroom/[id]/page.tsx:40-47](file://app/classroom/[id]/page.tsx#L40-L47)
- [lib/hooks/use-preview-timer.ts:23-76](file://lib/hooks/use-preview-timer.ts#L23-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)
- [middleware.ts:78-136](file://middleware.ts#L78-L136)

## 功能模块清单
- 根布局与全局上下文
  - 职责：提供主题、国际化、服务端初始化、访问码守卫与全局样式。
  - 关键点：在根布局中注入 AccessCodeGuard，统一拦截未授权访问。
- 首页（入口）
  - 职责：收集需求与资料，校验可用模型，准备会话并跳转预览生成。
  - 关键点：使用客户端路由进行导航；将生成参数持久化到 sessionStorage。
- 预览生成（中间态）
  - 职责：流式生成大纲与媒体任务，支持审阅与自动继续，完成后跳转教室。
  - 关键点：强制动态渲染，避免 SSR 不匹配；维护会话状态与重试策略。
- 教室详情（动态路由）
  - 职责：按课程 ID 加载场景、恢复未完成任务、渲染课堂并继续生成。
  - 关键点：使用 useParams 获取 id；清理跨课程污染状态；自动恢复媒体生成。
  - **新增**：集成预览限制功能，从URL参数提取courseSlug用于返回链接。
- **预览限制系统**
  - 职责：管理课堂内10分钟免费试看计时，超时后显示阻断覆盖层。
  - 关键点：使用localStorage存储访问时间戳，每秒更新倒计时状态。
  - **新增**：PreviewExpiredOverlay组件提供美观的超时提示界面。
- 中间件与安全
  - 职责：访问码鉴权、服务密钥放行、速率限制、安全响应头。
  - 关键点：区分高成本与通用 API 限流；对未认证 API 返回 401，页面放行由前端弹窗处理。
- 构建与部署
  - 职责：子路径部署、独立输出、外部包配置、CSP 等安全头。
  - 关键点：basePath 支持反向代理前缀；生产环境启用 HSTS。

**章节来源**
- [app/layout.tsx:27-48](file://app/layout.tsx#L27-L48)
- [app/page.tsx:317-409](file://app/page.tsx#L317-L409)
- [app/generation-preview/layout.tsx:1-7](file://app/generation-preview/layout.tsx#L1-L7)
- [app/generation-preview/page.tsx:187-200](file://app/generation-preview/page.tsx#L187-L200)
- [app/classroom/[id]/page.tsx:27-107](file://app/classroom/[id]/page.tsx#L27-L107)
- [lib/hooks/use-preview-timer.ts:23-76](file://lib/hooks/use-preview-timer.ts#L23-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)
- [middleware.ts:78-136](file://middleware.ts#L78-L136)
- [next.config.ts:1-40](file://next.config.ts#L1-L40)
- [components/access-code-guard.tsx:9-54](file://components/access-code-guard.tsx#L9-L54)

## 数据与状态
- 会话与参数传递
  - generationSession：首页写入，预览生成读取，包含需求、文档源、PDF 信息、当前步骤等。
  - generationParams：预览生成写入，教室详情页读取以恢复媒体映射与智能体配置。
  - **新增**：courseSlug URL参数用于返回链接，通过window.location.search提取。
- 预览限制状态管理
  - PreviewTimerState：包含expired布尔值和remainingSeconds数字，用于控制覆盖层显示。
  - localStorage存储：使用`openmaic.preview.{classroomId}`键值存储首次访问时间戳。
  - 计时逻辑：每秒计算剩余时间，超过10分钟设置expired=true并停止计时器。
- 状态隔离与清理
  - 切换教室时清理媒体任务与白板历史，防止跨课程污染。
  - 使用令牌机制确保并发加载与卸载时的正确性。
- URL 同步与历史操作
  - 使用 next/navigation 的 useRouter 进行 push 导航，保持浏览器历史一致。
  - 动态路由通过 [id] 参数驱动教室详情页面。
  - **新增**：URL查询参数courseSlug用于构建返回课程页面的链接。

```mermaid
flowchart TD
Start(["进入首页"]) --> Collect["收集需求与资料"]
Collect --> Validate{"可用模型存在?"}
Validate --> |否| Prompt["提示配置模型"]
Validate --> |是| Prepare["准备会话(generationSession)"]
Prepare --> Navigate["router.push('/generation-preview')"]
Navigate --> Preview["预览生成: 流式生成大纲/媒体"]
Preview --> Review{"是否需审阅?"}
Review --> |是| Edit["编辑/确认大纲"]
Review --> |否| AutoContinue["自动继续"]
Edit --> Continue["继续生成"]
AutoContinue --> Continue
Continue --> Classroom["router.push('/classroom/[id]?courseSlug=xxx')"]
Classroom --> Load["加载教室数据/恢复任务"]
Load --> InitTimer["初始化预览计时器(usePreviewTimer)"]
InitTimer --> TimerCheck["检查10分钟免费试看状态"]
TimerCheck --> Render["渲染课堂/继续生成媒体"]
Render --> CheckExpired{"是否超时?"}
CheckExpired --> |否| ContinuePlaying["继续播放"]
CheckExpired --> |是| ShowOverlay["显示PreviewExpiredOverlay"]
ShowOverlay --> BackLink["提供返回课程页面链接"]
BackLink --> End(["结束体验"])
```

**图表来源** 
- [app/page.tsx:317-409](file://app/page.tsx#L317-L409)
- [app/generation-preview/page.tsx:187-200](file://app/generation-preview/page.tsx#L187-L200)
- [app/classroom/[id]/page.tsx:40-47](file://app/classroom/[id]/page.tsx#L40-L47)
- [lib/hooks/use-preview-timer.ts:23-76](file://lib/hooks/use-preview-timer.ts#L23-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)

**章节来源**
- [app/page.tsx:317-409](file://app/page.tsx#L317-L409)
- [app/generation-preview/page.tsx:187-200](file://app/generation-preview/page.tsx#L187-L200)
- [app/classroom/[id]/page.tsx:40-47](file://app/classroom/[id]/page.tsx#L40-L47)
- [lib/hooks/use-preview-timer.ts:23-76](file://lib/hooks/use-preview-timer.ts#L23-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)

## 关键约束与边界
- 鉴权与访问控制
  - 中间件校验访问码 Cookie 与服务密钥，未认证 API 返回 401，页面请求放行由前端弹窗提示。
  - 访问码状态由前端组件轮询后端接口决定是否需要弹窗。
- 速率限制
  - 通用 API 每分钟 60 次，高成本 API（生成类）每分钟 10 次，超限返回 429 并附带 Retry-After。
- 安全头与部署
  - 设置 CSP、X-Frame-Options、Referrer-Policy、Permissions-Policy，生产环境启用 HSTS。
  - basePath 支持子路径部署；standalone 输出便于容器化部署。
- 渲染策略
  - 预览生成页强制动态渲染，避免 SSR 与客户端 hooks 的不匹配。
  - 根布局提供全局上下文，减少重复初始化开销。
- **预览限制约束**
  - 10分钟免费试看基于localStorage存储，清除缓存会重置计时。
  - 每个classroomId独立计时，重新打开课堂重新开始计时。
  - 超时后显示固定z-index(50)的覆盖层，阻止用户继续操作。
  - 提供可选的backUrl参数，优先显示"返回课程页"按钮而非关闭页面。

**章节来源**
- [middleware.ts:78-136](file://middleware.ts#L78-L136)
- [components/access-code-guard.tsx:9-54](file://components/access-code-guard.tsx#L9-L54)
- [next.config.ts:13-36](file://next.config.ts#L13-L36)
- [app/generation-preview/layout.tsx:1-7](file://app/generation-preview/layout.tsx#L1-L7)
- [app/layout.tsx:27-48](file://app/layout.tsx#L27-L48)
- [lib/hooks/use-preview-timer.ts:5-76](file://lib/hooks/use-preview-timer.ts#L5-L76)
- [components/preview-expired-overlay.tsx:18-75](file://components/preview-expired-overlay.tsx#L18-L75)