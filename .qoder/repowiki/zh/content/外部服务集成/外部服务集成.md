...
- TTS 调用流程：前端选择 TTS 提供商与音色/格式 → 服务端按提供商适配请求体与鉴权头 → 调用后端接口获取音频二进制 → 返回统一格式（音频字节数组与格式）。
...
- 文本转语音（TTS）提供商
  - 支持 OpenAI TTS、Azure TTS、GLM TTS、Qwen TTS、MiniMax TTS、豆包 TTS、ElevenLabs TTS、浏览器原生 Web Speech API、VoxCPM2（多后端）、Lemonade TTS。
  - 关键职责：提供商注册表、音色/格式/速度范围配置、请求体适配、错误分类与限流处理。
- 自动语音识别（ASR）提供商
...
- TTS/ASR 提供商
  - Lemonade：设置 LEMONADE_BASE_URL 及 TTS/ASR/IMAGE 专用 Base URL。
  - VoxCPM2：设置 TTS_VOXCPM_BASE_URL，无需 API Key。
- 图像/视频提供商
...
TTS["TTS 服务OpenAI/Azure/GLM/Qwen/MiniMax/豆包/ElevenLabs/Lemonade/VoxCPM2"]
...
### 文本转语音（TTS）提供商集成
- 提供商注册表：集中定义 TTS_PROVIDERS，包含名称、图标、默认 Base URL、模型与音色、支持格式与速率范围。
- 适配器实现：按 providerId 分发到具体实现（OpenAI、Azure、GLM、Qwen、MiniMax、豆包、ElevenLabs、Lemonade、VoxCPM2）。
- 请求适配：不同提供商的请求体与头部差异较大，适配器负责转换；返回统一格式（Uint8Array 音频与格式）。
- 错误分类：限流、鉴权失败、网络异常与服务端错误分类处理；读取错误详情并抛出明确错误。

```mermaid
flowchart TD
Entry(["generateTTS(config, text)"]) --> Switch["switch(providerId)"]
Switch --> OpenAI["OpenAI TTS"]
Switch --> Azure["Azure TTS"]
Switch --> GLM["GLM TTS"]
Switch --> Qwen["Qwen TTS"]
Switch --> MiniMax["MiniMax TTS"]
Switch --> Doubao["豆包 TTS"]
Switch --> ElevenLabs["ElevenLabs TTS"]
Switch --> Lemonade["Lemonade TTS"]
Switch --> VoxCPM["VoxCPM2"]
OpenAI --> Call["调用 API"]
Azure --> Call
GLM --> Call
Qwen --> Call
...
## 结论
OpenMAIC 的外部服务集成通过统一的提供商抽象层与注册表机制，实现了 LLM、TTS、ASR、图像与视频生成的灵活接入。配置与鉴权集中在常量与适配器中，错误处理与重试机制完善，便于扩展新的服务提供商与自定义实现。通过 SSRF 校验、内容安全过滤与用量记录，系统在安全性与可观测性方面具备良好实践。