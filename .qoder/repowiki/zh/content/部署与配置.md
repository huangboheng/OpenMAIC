...
- [scripts/start-dev.bat](file://scripts/start-dev.bat)
...
- [packages/@openmaic/renderer/src/elements/code/BaseCodeElement.tsx](file://packages/@openmaic/renderer/src/elements/code/BaseCodeElement.tsx)
</cite>

## 更新摘要
**所做更改**
- 新增了 PM2 开发环境配置章节，详细说明内存管理和自动重启机制
- 更新了开发环境部署部分，包含 PM2 进程管理配置
- 增强了内存优化和故障恢复策略说明
- 添加了 PM2 健康检查和迁移工具的使用指南
...
- Windows 启动脚本
  - 批处理脚本：`scripts/start-dev.bat`
  - PowerShell 脚本：`scripts/start-dev.ps1`
  - 自动检查中间件冲突和端口占用
...
- [scripts/start-dev.bat:1-38](file://scripts/start-dev.bat#L1-L38)
- [scripts/start-dev.ps1:1-35](file://scripts/start-dev.ps1#L1-L35)
...
- PM2 迁移工具
  - Python 迁移脚本：`scripts/pm2-migrate.py` 用于 PM2 配置迁移和验证
  - 批量操作：支持删除、重启、状态检查等操作
