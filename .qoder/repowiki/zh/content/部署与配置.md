...
- [.qoder-cn/mcp.json](file://.qoder-cn/mcp.json)
- [.mcp.json](file://.mcp.json)
</cite>

## 更新摘要
**所做更改**
- 新增了 MCP 配置版本锁定策略章节，详细说明生产环境稳定性保障机制
- 更新了依赖管理部分，强调固定版本的重要性
- 增强了生产环境部署的安全性和稳定性指导
- 添加了 MCP 运行器配置的最佳实践
...
**新增** 为确保生产环境的稳定性和可重复性，项目实施了严格的 MCP（Model Context Protocol）配置版本锁定策略。

### 版本锁定政策
- **固定版本要求**：所有 MCP 运行器必须锁定到已验证的固定版本，禁止使用 `@latest` 标签
- **版本验证**：每个版本都经过充分测试和验证，确保在生产环境中稳定运行
- **变更记录**：版本变更需要明确记录验证日期和变更原因

### 当前锁定版本
- **mcp-remote**: 0.1.38 - 用于远程 MCP 服务器连接
- **@plur-ai/mcp**: 0.16.1 - 用于 Plur AI 集成

### 配置文件结构
```json
{
  "_meta": {
    "versionPolicy": "所有 MCP 运行器锁定到已验证的固定版本，禁止使用 @latest",
  ...
  "mcpServers": {
    "keymetrics": {
      "command": "npx",
      "args": ["-y", "mcp-remote@0.1.38", "https://mcp.keymetrics.io/mcp"]
    },
    "plur": {
      "command": "npx",
      "args": ["-y", "@plur-ai/mcp@0.16.1"]
    }
  }
}
```
...
### 生产环境最佳实践
- **版本一致性**：确保所有部署环境使用相同的 MCP 版本
- **依赖管理**：通过配置文件统一管理第三方依赖版本
- **回滚策略**：保留历史版本配置以便快速回滚
- **监控告警**：监控 MCP 服务连接状态和性能指标

**章节来源**
- [.qoder-cn/mcp.json:1-21](file://.qoder-cn/mcp.json#L1-L21)
- [.mcp.json:1-18](file://.mcp.json#L1-L18)
...
- 配置备份
  - .env.local、server-providers.yml、.qoder-cn/mcp.json 等配置文件纳入版本管理与备份流程。
- PM2 配置备份
