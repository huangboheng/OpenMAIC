...
## 更新摘要
**所做更改**
- 移除了项目级别的 `.qoder-cn/mcp.json` 配置文件相关章节
- 更新了 MCP 配置管理策略，强调用户级别配置的重要性
- 增强了配置文件管理的最佳实践指导
- 更新了故障诊断工具部分，移除了对已删除文件的引用
...
**更新** 配置文件管理策略已更新，移除了项目级别的 MCP 配置文件。

### 配置文件分类
- **项目级配置**：`.env.local`、`server-providers.yml`、`ecosystem.dev.config.cjs`
- **用户级配置**：`~/.qoder-cn/` 目录下的个人配置
- **忽略文件**：`.gitignore` 中明确排除的个人配置文件

### MCP 配置管理
- **移除项目级配置**：不再在项目根目录维护 `.qoder-cn/mcp.json` 文件
- **用户级配置优先**：所有 MCP 相关配置移至用户级别目录 `~/.qoder-cn/`
- **配置文件忽略**：`.mcp.json` 文件已在 `.gitignore` 中排除

### 配置文件最佳实践
- **敏感信息保护**：API 密钥和个人配置不应提交到版本控制系统
- **配置模板管理**：使用 `.env.example` 提供配置模板
- **环境分离**：开发、测试、生产环境使用不同的配置文件
- **配置验证**：启动时验证配置文件的完整性和有效性

**章节来源**
- [.gitignore:107-109](file://.gitignore#L107-L109)