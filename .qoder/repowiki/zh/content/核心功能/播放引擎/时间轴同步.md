...
- [tests/lib/choreography/timing.test.ts](file://tests/lib/choreography/timing.test.ts)
</cite>

## 更新摘要
**变更内容**   
- 新增课堂总时长估算算法章节，详细说明 classroom-generation.ts 中的综合时长计算逻辑
- 更新语音阅读时间估算算法，区分中文（4字/秒）和英文（2词/秒）的不同语速
- 扩展白板动作固定时长支持，包括绘图、代码编辑和视频播放等动作类型
- 增强时间轴同步机制，确保应用与导出视频的一致性
...
**更新** 新增了课堂总时长估算算法，通过综合分析场景中的 speech 动作和白板动作，为整个课堂生成准确的预计时长。
...
- 课堂时长计算：在课堂生成阶段，综合分析所有场景的动作，计算总时长用于课程元数据展示。
...
- 课堂总时长估算
  - 职责：在课堂生成阶段分析所有场景动作，计算总时长用于课程元数据。
  - 验收要点：中文文本按4字/秒，英文按2词/秒，白板动作固定2秒估算。
- 场景时间轴支持判定
...
- [lib/server/classroom-generation.ts:546-571](file://lib/server/classroom-generation.ts#L546-L571)
...
- 时间精度与延迟
  ...
  - 课堂总时长估算：中文 4字/秒，英文 2词/秒，白板动作固定 2秒。
- 依赖与集成边界
...
### 课堂总时长估算算法
**新增** 在课堂生成阶段，系统会综合分析所有场景的动作来计算总时长，用于课程元数据的展示。

- 估算规则
  - Speech 动作：中文文本按 4 字/秒计算，英文文本按 2 词/秒计算。
  - 白板动作：wb_open、wb_draw_text、wb_draw_shape、wb_draw_chart、wb_draw_latex、wb_draw_table、wb_draw_line、wb_draw_code、wb_edit_code、wb_clear、wb_delete、wb_close 等动作固定 2 秒。
  - 其他动作：discussion、play_video 等动作也固定 2 秒。
  - 总时长：将所有动作时长累加后转换为分钟（向上取整）。

- 实现细节
  - 遍历所有场景的所有动作
  - 根据动作类型应用不同的估算策略
  - 将秒数转换为分钟并向上取整
  - 设置 stage.totalDuration 和 stage.estimatedDuration

```mermaid
flowchart TD
Start(["输入场景数组"]) --> LoopScenes["遍历每个场景"]
LoopScenes --> LoopActions["遍历场景中的动作"]
LoopActions --> CheckType{"动作类型判断"}
CheckType --> |speech| CalcSpeech["统计中英文字符数<br/>中文: 字符数/4<br/>英文: 词数/2"]
CheckType --> |白板动作| Fixed2s["固定 2 秒"]
CheckType --> |其他动作| Fixed2s
CalcSpeech --> AddDuration["累加时长"]
Fixed2s --> AddDuration
AddDuration --> NextAction{"还有动作?"}
NextAction --> |是| LoopActions
NextAction --> |否| NextScene{"还有场景?"}
NextScene --> |是| LoopScenes
NextScene --> |否| ConvertMin["转换为分钟(向上取整)"]
ConvertMin --> End(["输出总时长"])
```
...
**图表来源** 
- [tests/playback/action-navigation.test.ts:261-290](file://tests/playback/action-navigation.test.ts#L261-L290)
- [tests/playback/action-navigation.test.ts:382-431](file://tests/playback/action-navigation.test.ts#L382-431)
- [tests/playback/action-navigation.test.ts:221-252](file://tests/playback/action-navigation.test.ts#L221-252)

**章节来源**
- [tests/playback/action-navigation.test.ts:261-290](file://tests/playback/action-navigation.test.ts#L261-290)
- [tests/playback/action-navigation.test.ts:382-431](file://tests/playback/action-navigation.test.ts#L382-431)
- [tests/playback/action-navigation.test.ts:221-252](file://tests/playback/action-navigation.test.ts#L221-252)
...
**图表来源** 
- [lib/video-export/compile.ts:131-155](file://lib/video-export/compile.ts#L131-155)
- [lib/video-export/passes/assets.ts:202-229](file://lib/video-export/passes/assets.ts#L202-229)
...
**章节来源**
- [components/edit/scene-timeline.ts:10-17](file://components/edit/scene-timeline.ts#L10-17)

## 性能考虑
- 估算与计算
  - 朗读时长估算为线性复杂度，适合高频调用；避免重复创建正则对象。
  - 课堂总时长估算为 O(n×m) 复杂度，n 为场景数，m 为动作数，在生成阶段一次性计算。
- 媒体等待上限
...
- 课堂时长估算不准确
  - 现象：生成的课程时长与实际播放时长不符。
  - 原因：动作类型识别错误或估算公式不一致。
  - 处理：检查 classroom-generation.ts 中的动作类型判断逻辑和估算公式。

**章节来源**
- [tests/playback/action-navigation.test.ts:433-473](file://tests/playback/action-navigation.test.ts#L433-473)
- [lib/video-export/passes/assets.ts:202-229](file://lib/video-export/passes/assets.ts#L202-229)
- [lib/choreography/timing.ts:16-34](file://lib/choreography/timing.ts#L16-34)
- [lib/server/classroom-generation.ts:546-571](file://lib/server/classroom-generation.ts#L546-571)

## 结论
OpenMAIC 的时间轴同步通过统一的 timing 规范与估算函数，确保应用与导出的一致性；回放引擎对动作导航、媒体同步与特效生命周期进行严格管控，保障视觉、语音与动画的精确对齐。新增的课堂总时长估算算法通过分析 speech 动作和白板动作，为课程提供准确的预计时长。通过合理的等待上限、过时回调守卫与资产计划，系统在复杂场景下仍能保持稳定与高性能。建议在扩展新动作类型时，遵循现有规范与约束，保持时间轴的一致性与可预测性。