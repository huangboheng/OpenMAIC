...
## 更新摘要
**所做更改**   
- 增强了CDN弹性场景下的错误处理和调试能力
- 新增了运行时错误捕获和恢复机制的详细说明
- 更新了iframe错误shim的错误缓冲和重放功能
- 增加了CDN不可用时的降级策略说明
- 完善了故障排查指南，包含CDN相关问题的诊断方法
...
**最新更新**：系统现在具备更强的CDN弹性能力，当外部依赖（如KaTeX CDN）不可用时，能够自动降级并继续运行，同时提供完善的错误捕获和调试信息。
...
- [InteractiveIframeHost.tsx:100-117](file://components/scene-renderers/InteractiveIframeHost.tsx#L100-117)
...
- [widget-iframe.ts:1-50](file://lib/store/widget-iframe.ts#L1-L50)
- [InteractiveIframeHost.tsx:32-75](file://components/scene-renderers/InteractiveIframeHost.tsx#L32-75)
- [interactive-renderer.tsx:22-69](file://components/scene-renderers/interactive-renderer.tsx#L22-69)
...
- [widget-iframe.ts:27-50](file://lib/store/widget-iframe.ts#L27-L50)
- [InteractiveIframeHost.tsx:100-117](file://components/scene-renderers/InteractiveIframeHost.tsx#L100-117)
- [interactive-renderer.tsx:33-69](file://components/scene-renderers/interactive-renderer.tsx#L33-69)
...
**更新**：系统现在具备增强的CDN弹性能力和更完善的错误处理机制。

#### 运行时错误捕获机制
- **错误捕获Shim**：iframe 注入 ERROR_CAPTURE_SHIM，在页面脚本执行前捕获所有运行时错误。
- **错误缓冲**：支持错误缓冲（最多50条），解决父页面订阅前的错误丢失问题。
- **错误重放**：父页面订阅后请求重放，恢复早期错误，避免漏报。
- **错误分类**：支持 error、resource、unhandledrejection、console.error 等多种错误类型。

#### CDN弹性降级策略
- **KaTeX CDN检测**：自动检测 KaTeX CDN 加载状态，不可用时提供无操作降级。
- **非阻塞加载**：将传统的阻塞式 `<script src>` 转换为异步动态加载。
- **容错处理**：即使 CDN 完全不可用，widget 仍能正常渲染和交互。

#### 错误存储与管理
- **场景隔离**：按 sceneId 组织错误信息，避免不同场景间的干扰。
- **去重机制**：自动去除重复错误，避免日志膨胀。
- **容量限制**：每个场景最多保留8条错误，保持上下文精简。

```mermaid
sequenceDiagram
participant Parent as "父页面"
participant Iframe as "iframe"
participant ErrorShim as "错误捕获Shim"
participant Store as "错误存储"
Parent->>Iframe : 创建iframe并注入shim
Iframe->>ErrorShim : 执行错误捕获shim
ErrorShim->>ErrorShim : 缓存早期错误(最多50条)
Parent->>Iframe : 订阅message事件
Iframe->>Parent : 请求错误重放
Parent->>Iframe : 发送重放请求
Iframe->>Parent : 重放缓存的错误
Parent->>Store : 存储错误信息(去重、限流)
```
...
- [scene-runtime-errors.ts:23-42](file://lib/store/scene-runtime-errors.ts#L23-L42)

**章节来源**
- [InteractiveIframeHost.tsx:125-139](file://components/scene-renderers/InteractiveIframeHost.tsx#L125-L139)
- [scene-runtime-errors.ts:1-43](file://lib/store/scene-runtime-errors.ts#L1-L43)
- [iframe.ts:36-99](file://lib/utils/iframe.ts#L36-L99)
...
#### CDN相关问题
- **现象**：页面空白或数学公式无法渲染
- **原因**：cdn.jsdelivr.net 不可用或网络受限
- **解决方案**：系统会自动降级，widget 仍可正常使用基础功能
- **诊断**：查看错误日志中的 "Failed to load resource" 信息

#### 错误信息缺失
- **现象**：编辑器看不到iframe中的错误信息
- **原因**：错误发生在父页面订阅message事件之前
- **解决方案**：系统会自动请求错误重放，恢复早期错误
- **检查**：确认 useSceneRuntimeErrors 中的错误列表

**章节来源**
- [system.md:53-122](file://lib/prompts/templates/diagram-content/system.md#L53-L122)
- [scene-runtime-errors.ts:1-43](file://lib/store/scene-runtime-errors.ts#L1-L43)
- [interactive-renderer.tsx:33-69](file://components/scene-renderers/interactive-renderer.tsx#L33-69)
- [iframe.test.ts:118-150](file://tests/lib/utils/iframe.test.ts#L118-L150)

## 结论
OpenMAIC 的组件交互动作体系以 ActionEngine 为核心，通过标准化的 postMessage 协议与稳定的 iframe 池管理，实现了高效、安全、可观测的 widget 控制能力。**最新更新**增强了系统的CDN弹性能力，即使在外部依赖不可用的情况下也能保证基本功能的正常运行，同时提供了完善的错误捕获和调试机制。遵循本文档的消息格式、回调注册与错误处理规范，开发者可以快速构建高质量的交互式课件，并在复杂课堂场景中保持流畅的用户体验。